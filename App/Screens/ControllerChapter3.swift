...
class ControllerChapter3: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let choices: [choice] = choice.allChoices3
...
Когда вы записываетесь на курсы или покупаете что-то, важно, чтобы организаторы дали вам информацию о себе (их контактные данные, например) и о том, сколько вы заплатили (чек об оплате). Это нужно для того, чтобы всё было честно и законно. Предоставление реквизитов организаторов курса и чека об оплате может быть обязательным в соответствии с законодательством о защите прав потребителей или в соответствии с договором об оказании услуг.


Комментарий 2:

Когда ты покупаешь информационный продукт, такой как программное обеспечение или электронная книга, на тебя распространяется законы о защите прав потребителей и договоры купли-продажи. Это означает, что у тебя есть право получить точную и полную информацию о продукте, его цене, условиях использования, а также возможность вернуть его или обменять, если он не соответствует твоим ожиданиям или имеет дефекты.
""")
    ...
    func goToNextScene() {
        ...
        if currentSceneId == 22 && !not_peace {
            nextSceneId = 26
        } else if currentSceneId == 54 && !director{
            nextSceneId = 61
        }
        if nextSceneId == -1 {
            saveData.numScene = 0
